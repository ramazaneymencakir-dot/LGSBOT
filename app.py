...
def soru_kontrol(ders, konu, tur, soru):
    ...
    prompt = f"""
Sen çok sıkı bir LGS soru denetçisisin.
...
{json.dumps(soru, ensure_ascii=False)}

Şunları tek tek kontrol et:

1. Soru gerçekten seçilen konuya mı ait?
2. Seçilen konuyu bilmeden soru çözülebiliyor mu?
   Çözülebiliyorsa REDDET.
3. Soru 8. sınıf seviyesinde mi?
4. Soru kendi içinde mantıklı mı?
5. Verilen bilgiler çelişkili mi?
6. Yalnızca bir doğru seçenek var mı?
7. Verilen cevap gerçekten doğru mu?
8. Çözüm gerçekten doğru mu?
9. LGS Tarzı seçildiyse gerçekten muhakeme gerektiriyor mu?
10. Konuyla ilgisiz başka bir kazanım sorunun merkezine geçmiş mi?

Önemli bir sorun varsa onaylama.

SADECE JSON döndür:

{{
    "onay": true,
    "neden": "kontrol sonucu"
}}
"""
...
def guvenli_soru_uret(ders, konu, tur):
